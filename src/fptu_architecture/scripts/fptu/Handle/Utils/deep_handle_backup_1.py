...
class segment:
    ...
    def onehot_to_rgb(self,onehot, colormap):
        ...
        if sign_density > 200:
            ...
            #
            centroid_x_sign,centroid_y_sign = compute_sign(sign)
            centroids = np.array([centroid_x_sign,centroid_y_sign,sign_density], dtype=np.float32)
    ...
    while not rospy.is_shutdown():
        ...
        if read.frame is not None:
            ...
            try:
                ...
                print("HELOOOOOOOOOOOOOOOOOOO",sign_datvu)# Edit content
                ...
                # Huy Phan 01/06 TEST
                if sign_bool == 69:        
                    if 0 <= sign_density < 30:
                        time_nl_turn=time.time()
                        while time.time()- time_nl_turn <= 0.5:
