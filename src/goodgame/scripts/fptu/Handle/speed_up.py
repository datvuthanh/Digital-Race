...
def angle_calculator(x,y):
    ...
    slope = (x - 72) / float(y - 144) # (50,128) is centroid of image size 100x128
...
@njit(fastmath=True)
def remove_lane_lines(road,bamlan):
  min_x = 255
  max_x = -1
    ...
    if bamlan == 1:
      for k in range(0,centroid_x_new):  
        if road[i][k] == 255:
          road[i][k] = 0
    if bamlan == -1:
      for k in range(centroid_x_new,road.shape[1]):  
        if road[i][k] == 255:
          road[i][k] = 0
...
def cut_road_speedup(road,width):
  for i in range(road.shape[0]):
      for j in range(road.shape[1]):
          if j > width:
              road[i][j] = 0
...
def cut_road_speedup_for_mid(road,width):
    for i in range(road.shape[0]):
        for j in range(road.shape[1]):
            if j < width:
                road[i][j] = 0
...
def count_line(road):
    ...
    for k in range(20,120):
        for j in range(road.shape[0]):
            if road[j][k] == 255:
                counting_line += 1
        #print("COUNTING_LINE: ",counting_line)
        if counting_line > max_line:
            max_line = counting_line
